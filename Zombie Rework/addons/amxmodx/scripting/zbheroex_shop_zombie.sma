...
new const ModelV[9][] =
{
	"models/zombie_evolution/zombie/v_grenade_regular.mdl",
	"models/zombie_evolution/zombie/v_grenade_light.mdl",
	"models/zombie_evolution/zombie/v_grenade_voodoo.mdl",
	"models/zombie_evolution/zombie/v_grenade_deimos_host.mdl",
	"models/zombie_evolution/zombie/v_grenade_deimos_host.mdl",
	"models/zombie_evolution/zombie/v_grenade_banshee.mdl",
	"models/zombie_evolution/zombie/v_grenade_banshee.mdl",
	"models/zombie_evolution/zombie/v_grenade_stamper.mdl",
	"models/zombie_evolution/zombie/v_grenade_stingfinger.mdl"
}
...
public plugin_precache()
{
	...
	for(new i = 0; i < sizeof(ModelV); i++)
		engfunc(EngFunc_PrecacheModel, ModelV[i])	
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_HEGRENADE && g_OldWeapon[id] != CSW_HEGRENADE))
	{
	...
	} else if((CSWID == CSW_FLASHBANG && g_OldWeapon[id] != CSW_FLASHBANG))
	{
		set_pev(id, pev_viewmodel2, ModelV[Get_ZombieGrenade_ModelId(zbheroex_get_user_zombie_class(id), zbheroex_get_user_level(id) > 1 ? 1 : 0)])
		set_pev(id, pev_weaponmodel2, ModelW)
	}
...
public fw_SetModel(Ent, const Model[])
{
	...
	if(is_user_connected(id) && zbheroex_get_user_zombie(id))
	{
		...
		} else if(Model[9] == 'f' && Model[10] == 'l') // Zombie Bomb 2
		{
			...
			set_pev(Ent, pev_flTimeStepSound, 3465346)
...
public fw_ThinkGrenade(Ent)
{
	...
	if(dmgtime - 2.0 <= get_gametime())
	{
		...
		if(is_user_alive(Victim) && pev(Ent, pev_flTimeStepSound) == 3465346)
		{
	...
	if(pev(Ent, pev_flTimeStepSound) == 3465345)
	{
		ZombieGrenade_Explosion(Ent)
		return HAM_SUPERCEDE
	} else if(pev(Ent, pev_flTimeStepSound) == 3465346) {
		ZombieGrenade_Explosion(Ent)
		return HAM_SUPERCEDE
	}
