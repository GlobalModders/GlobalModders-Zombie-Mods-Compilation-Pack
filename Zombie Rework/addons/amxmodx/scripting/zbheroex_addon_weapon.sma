...
public plugin_natives()
{
	...
	register_native("zbheroex_supplybox_weapon", "Native_OpenWeapon", 1)
}
...
public Native_OpenWeapon(id)
{
	...
	if(zbheroex_get_user_hero(id) || zbheroex_get_user_zombie(id)) return
	
	if(!is_user_bot(id))
	{
		// Ask
		static LangText[64]; formatex(LangText, 63, "Open Weapon Menu ?")
		static Menu; Menu = menu_create(LangText, "MenuHandle_MileageAsk")
		
		// Yes
		formatex(LangText, 63, "Yes")
		menu_additem(Menu, LangText, "yes")
		
		// No
		formatex(LangText, 63, "No")
