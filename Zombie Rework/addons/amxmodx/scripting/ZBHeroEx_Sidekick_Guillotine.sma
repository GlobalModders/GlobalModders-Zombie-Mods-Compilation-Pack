...
#define PLUGIN "[ZEVO] Sidekick: Guillotine"
#define VERSION "1.0"
#define AUTHOR "Dias Pendragon"

#define DAMAGE 200 // 35: Human | 200: Zombie
#define AMMO 10
...
#define MODEL_V "models/zombie_evolution/swpn/v_guillotine.mdl"
#define MODEL_P "models/zombie_evolution/swpn/p_guillotine.mdl"
#define MODEL_W "models/zombie_evolution/swpn/w_guillotine.mdl"

#define MODEL_S "models/zombie_evolution/swpn/guillotine_projectile.mdl"
#define MODEL_GIB "models/zombie_evolution/swpn/gibs_guilotine.mdl"
...
public plugin_init()
{
	...
	g_Guillotine = zbheroex_register_specialweapon("Blood Dripper", HUMAN_SIDEKICK)
	register_clcmd("dias_get_guillotine", "Get_Guillotine", ADMIN_ADMIN)
...
public plugin_precache()
{
	...
	g_ExpSprID = precache_model(Weapon_Resources[0])
}
...
public zbheroex_specialweapon(id, PlayerType, ItemID)
{
	if(ItemID == g_Guillotine) Get_Guillotine(id)
}

public zbheroex_specialweapon_refill(id, ItemID)
{
	if(ItemID == g_Guillotine)
	{
		g_Ammo[id] = AMMO
		if(get_user_weapon(id) == CSW_GUILLOTINE) update_ammo(id, -1, AMMO)
	}
}

public zbheroex_specialweapon_remove(id, ItemID)
{
	if(ItemID == g_Guillotine) Remove_Guillotine(id)
}
