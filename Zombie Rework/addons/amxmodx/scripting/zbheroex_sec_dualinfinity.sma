...
#define PLUGIN "[ZBHeroEx] Secondary: Dual Infinity"
#define VERSION "1.0"
#define AUTHOR "Dias"

#define V_MODELA "models/zombie_thehero/weapon/sec/v_infinityex2a.mdl"
#define V_MODELB "models/zombie_thehero/weapon/sec/v_infinityex2b.mdl"
#define P_MODEL "models/zombie_thehero/weapon/sec/p_infinityex2.mdl"
#define W_MODEL "models/zombie_thehero/weapon/sec/w_infinityex2.mdl"

#define CSW_DI CSW_USP
#define weapon_di "weapon_usp"

#define ANIM_EXT "dualpistols_1"
#define WEAPON_SECRETCODE 1952
...
enum
{
	...
	DI_ANIM_SHOOT_LEFT4,
	DI_ANIM_SHOOT_LEFT5,
	DI_ANIM_SHOOT_LEFTLAST,
	DI_ANIM_SHOOT_RIGHT1,
	DI_ANIM_SHOOT_RIGHT2,
	DI_ANIM_SHOOT_RIGHT3,
	DI_ANIM_SHOOT_RIGHT4,
	DI_ANIM_SHOOT_RIGHT5,
	DI_ANIM_SHOOT_RIGHTLAST,
	DI_ANIM_RELOAD,
	DI_ANIM_DRAW,
	DI_ANIM_CHANGE
}
...
new g_DualInfinity
new g_Had_DualInfinity, g_InSpecialShoot, g_InRapidShoot, g_Shooting, Float:g_LastShoot[33]
...
{
	engfunc(EngFunc_PrecacheModel, V_MODELA)
	...
	g_DualInfinity = zbheroex_register_weapon("Dual Infinity", WEAPON_SECONDARY, CSW_USP, 4999)
}
...
public Event_CurWeapon(id)
{
	...
	{
		if(!Get_BitVar(g_InSpecialShoot, id)) set_pev(id, pev_viewmodel2, V_MODELA)
		else set_pev(id, pev_viewmodel2, V_MODELB)
		set_pev(id, pev_weaponmodel2, P_MODEL)
		
		// Draw Scene
		set_weapon_anim(id, DI_ANIM_DRAW)
	...
	} else if(Get_BitVar(g_Had_DualInfinity, id) && CSWID == CSW_DI) {
		if(!Get_BitVar(g_InSpecialShoot, id)) set_pev(id, pev_viewmodel2, V_MODELA)
		else set_pev(id, pev_viewmodel2, V_MODELB)
	}
...
public Weapon_Attack(id)
{
	...
	{	
		...
		set_weapon_anim(id, random_num(DI_ANIM_SHOOT_LEFT1, DI_ANIM_SHOOT_LEFT5))
		make_shell(id, 0)
		...
		set_weapon_anim(id, random_num(DI_ANIM_SHOOT_RIGHT1, DI_ANIM_SHOOT_RIGHT5))
		make_shell(id, 1)
...
public Check_Recoil(id, Right)
{
	if(!Get_BitVar(g_InSpecialShoot, id))
	{
		...
		Push[0] = random_float(-0.2, -0.5)
		if(Right) Push[1] = random_float(0.1, 0.5)
		else Push[1] = random_float(-0.1, -0.5)
...
public fw_Weapon_Reload_Post(ent)
{
	...
	{ // Reload
		...
		set_weapon_anim(id, DI_ANIM_RELOAD)
...
public fw_Weapon_WeaponIdle_Post(ent)
{
	...
	{
		set_weapon_anim(id, DI_ANIM_IDLE)
