...
#define PLUGIN "[ZEVO] Sidekick: Speargun"
#define VERSION "1.0"
...
#define SPEAR_DAMAGE 560 // 95: Human | 560: Zombie
...
#define V_MODEL "models/zombie_evolution/swpn/v_speargun.mdl"
#define P_MODEL "models/zombie_evolution/swpn/p_speargun.mdl"
#define W_MODEL "models/zombie_evolution/swpn/w_speargun.mdl"
#define S_MODEL "models/zombie_evolution/swpn/s_spear.mdl"

new const SpearSounds[4][] = 
{
	"weapons/speargun-1.wav",
	"weapons/speargun_stone1.wav",
...
new g_SpearID
new g_TempAttack, g_Shoot
new g_Had_Spear, g_Spear[33], Float:CheckDelay[33], g_CurrentSpear[33], g_PreAmmo[33], g_MyOldWeapon[33]
...
public plugin_init() 
{
	...
	g_SpearID = zbheroex_register_specialweapon("Speargun", HUMAN_SIDEKICK)
	register_clcmd("weapon_speargun", "Hook_Weapon")
	register_clcmd("dias_get_speargun", "Get_Spear", ADMIN_ADMIN)
}
...
{
	if(ItemID == g_SpearID) Get_Spear(id)
}
...
{
	if(ItemID == g_SpearID) Remove_Spear(id)
}
...
public Event_CurWeapon(id)
{
	...
	if(CSWID != CSW_SPEAR && g_MyOldWeapon[id] == CSW_SPEAR && Get_BitVar(g_Had_Spear, id))
	{
		cs_set_user_bpammo(id, CSW_SPEAR, g_PreAmmo[id])
	} else if(CSWID == CSW_SPEAR && g_MyOldWeapon[id] != CSW_SPEAR && Get_BitVar(g_Had_Spear, id))
	{
		g_PreAmmo[id] = cs_get_user_bpammo(id, CSW_SPEAR)
		update_ammo(id, -1, g_Spear[id])
	} else if(CSWID == CSW_SPEAR && Get_BitVar(g_Had_Spear, id)) {
		update_ammo(id, -1, g_Spear[id])
	}
...
public fw_UpdateClientData_Post(id, sendweapons, cd_handle)
{
	...
	if(get_player_weapon(id) == CSW_SPEAR && Get_BitVar(g_Had_Spear, id))
...
public Spear_Shooting(id)
{
	...
	emit_sound(id, CHAN_WEAPON, SpearSounds[0], 1.0, 0.4, 0, 94 + random_num(0, 15))
	...
	if(is_connected(Id)) 
	{
		...
		if(is_alive(Target))
		{
			...
			if(Hitgroup == HIT_HEAD) 
			{
				...
				ExecuteHamB(Ham_TakeDamage, Target, 0, Id, float(SPEAR_DAMAGE) * 4.0, DMG_BURN)
			} else {
