...
public plugin_init()
{
	...
	g_Forwards[WPN_BOUGHT] = CreateMultiForward("gf_weapon_bought", ET_IGNORE, FP_CELL, FP_CELL)
	g_Forwards[WPN_REMOVE] = CreateMultiForward("gf_weapon_remove", ET_IGNORE, FP_CELL, FP_CELL)
	g_Forwards[WPN_ADDAMMO] = CreateMultiForward("gf_weapon_addammo", ET_IGNORE, FP_CELL, FP_CELL)
	
	g_MsgSayText = get_user_msgid("SayText")
	g_MaxPlayers = get_maxplayers()
	
	register_clcmd("ghostfury_equipment", "Show_MainEquipMenu")
}
...
{
	register_native("gf_weapon_register", "Native_RegisterWeapon", 1)
	register_native("gf_weapon_get_cswid", "Native_Get_CSWID", 1)
}
...
public gf_user_transghost_pre(id, killer, transformation)
{
...
public MenuHandle_MainEquip(id, Menu, Item)
{
	...
	if(!is_user_alive(id) || gf_get_user_ghost(id))
	{
		menu_destroy(Menu)
		return PLUGIN_HANDLED
	}
